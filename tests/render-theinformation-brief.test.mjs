...
test("renderer outputs the new Chinese brief section structure", () => {
  ...
  assert.match(text, /Amazon's AI Chat Ads Yield Data but Few Sales/);
  assert.match(text, /亚马逊 AI 聊天广告带来了数据，但几乎没带来销量/);
  ...
  assert.match(text, /### 算法窥探与反馈/);
  assert.doesNotMatch(text, /运行机制与营销者洞察/);

  assert.match(
    html,
    /<h2>Amazon's AI Chat Ads Yield Data but Few Sales<\/h2>\s*<p class="title-translation">亚马逊 AI 聊天广告带来了数据，但几乎没带来销量<\/p>/
  );
